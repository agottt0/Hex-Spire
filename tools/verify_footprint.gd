...
##
## ✓ 正解：通过性 = (anchor, facing) 状态图上从起点到目标存在路径。
##   必须用 HexPathfinder 搜索。**已实现并验证：tools/verify_pathfinding.gd**
##
##   实测结论（三级闸门，干净可用）：
##       门宽    S      M       L
##       1 格   可过   过不去   过不去    ← 真正的体型闸门
##       2 格   可过   可过     过不去    ← 巨人的舞台
##       3 格   可过   可过     可过
##   ⚠️ 注意 2 格门【能挡住 L】。我曾据静态几何断言"L 能骑过 2 格门"，那是错的：
##      L 在 2 格门确实存在合法静态姿态，但走不过去 —— 再次印证静态检查判不了通过性。
##
